...
namespace Terraria
{
	...
	public class Rain
	{
		...
		// Token: 0x060001F0 RID: 496 RVA: 0x000BF830 File Offset: 0x000BDA30
		public void Update()
		{
			...
			if (Collision.SolidCollision(this.position, 2, 2) || this.position.Y > Main.screenPosition.Y + (float)Main.screenHeight + 100f || Collision.WetCollision(this.position, 2, 2))
			{
				...
				if ((float)Main.rand.Next(100) < Main.gfxQuality * 100f)
				{
					...
					Dust expr_EB_cp_0_cp_0 = Main.dust[num2];
					expr_EB_cp_0_cp_0.position.X = expr_EB_cp_0_cp_0.position.X - 2f;
